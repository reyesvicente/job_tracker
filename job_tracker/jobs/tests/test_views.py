...
from job_tracker.jobs.views import ApplicationListView, ApplicationDetailView, ApplicationCreateView, ApplicationUpdateView, ApplicationDeleteView  # Use absolute import
...
def test_application_views(client):
    ...
    assert 'applications' in response.context, f"Expected 'applications' in context, but context was: {response.context}"
    ...
    response = client.post(reverse('jobs:add'), {
        'name': 'Test Name',
    ...
    response = client.post(reverse('jobs:update', kwargs={'slug': application.slug}), {
        'name': 'Updated Name',
